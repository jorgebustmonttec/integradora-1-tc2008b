...
public class SliderCon : MonoBehaviour
{
    ...
    {
        // This function triggers a POST request to the server with the current parameters
        // Adjusted to not directly interact with GridPlacer
        var parameters = new
        {
        ...
        StartCoroutine(NetworkManager.Instance.PostRunModel(jsonParameters, success =>
        {
            if (success)
            {
                ...
                // No direct call to GridPlacer here, only logging
            }
            else
            {
